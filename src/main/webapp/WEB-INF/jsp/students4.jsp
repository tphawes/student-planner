...
<body>
<div class="tab">
  <button class="tablinks" onclick="openCalendar()">Calendar</button>
  <button class="tablinks" onclick="openStudents()">Students</button>
  <button class="tablinks" onclick="openOther()">Other</button>
</div>

<div class="otherTab" id="otherArea">
	<button onclick="startFunction()" id="startButton">start button</button>
	<button onclick="openStudentForm()" id="formButton">form button</button>
	<button onclick="closeStudentForm()" id="formButton">form button2</button>
	<button onclick="loadStudentMeetingTable()" id="testButton">loadStudentMeetingTable()</button>
	<button onclick="loadCalendarTable2()" id="testButton">loadCalendarTable2</button>
		<br></br>
		<button onclick="fn1()" id="testButton">f1</button>
	<button onclick="fn2()" id="testButton">f2</button>
	<button onclick="getSelectedMeetingId()" id="testButton">get</button>
	<button onclick="setSelectedMeetingId(9)" id="testButton">set</button>
	
	<br></br>
	Selected Student<input type="text" name="status" id="selectedStudentID"/><br>
	Selected Meeting ID<input type="text" name="status" id="selectedMeetingID"/><br>
	Selected Meeting Time<input type="text" name="status" id="selectedMeetingTime"/><br>
	Selected Meeting Date<input type="text" name="status" id="selectedMeetingDate"/><br>
	...
	<form class="form-popup" id="studentInputForm" onSubmit="addStudent(); return false;" hidden>
		<h2>Student Data</h2>
		First Name : <input type="text" name="name" id="inputFirstName"/><br>
		Last Name : <input type="text" name="name" id="inputLastName"/><br>
		DOB : <input type="text" name="dob" id="inputDOB"/><br>
		GRADE : <input type="text" name="grade" id="inputGrade"/><br>
		STATUS : <input type="text" name="status" id="inputStatus"/><br>
		
		
		<input type="button" id ="addStudentFormButton" value="Add" onClick="updateStudent()"/>
		<input type="button" value="Cancel" onClick="closeStudentForm()"/>
	</form>

<div class="studentTab" id="studentArea">
<button onclick="openStudentForm()" value="Add Student" id="addStudentButton">Add Student</button>
...
<h2>Meeting Data</h2>
	<table id="student-meeting-table" class="table-layout" style="width: 100%" border='1'>
		<thead>
    	<tr>
      		<th>Date</th>
      		<th>Time</th>
...
<div class="calendarTab" id="calendarArea">
<h2>Weekly Calendar</h2>
Set Calendar Dates:<input type="date" id="dateInput" onChange=setCalendarDates()><br>
<table class="meeting_fixed_header" id="calendar-table2">
  <thead>
    	<tr>
    		<th ></th>
      		<th class="col-2">Monday</th>
			<th class="col-3">Tuesday</th>
			<th class="col-4">Wednesday</th>
      		<th class="col-5">Thursday</th>
      		<th class="col-6">Friday</th>
    	</tr>
    	<tr>
    		<th class="col-1">Time</th>
      		<th class="col-2" id = "WD1">TBD</th>
			<th class="col-3" id = "WD2">TBD</th>
			<th class="col-4" id = "WD3">TBD</th>
      		<th class="col-5" id = "WD4">TBD</th>
      		<th class="col-6" id = "WD5">TBD</th>
    	</tr>
  	</thead>
  	<tbody>
  	</tbody>
...
<div class="form-popup" id="calendarForm">
...
<br />
    <button type="button" class="btn cancel" onclick="closeCalendarForm()">Cancel</button>
    <button type="button" onclick="deleteMeeting()" id="deleteMeetingButton">DeleteMeeting</button>
    <button type="button" class="btn add" id="addUpdateMeetingButton" value="Add" onclick="addUpdateMeeting()"></button>
  </form>
</div>

    <script id="f1" type="text/javascript">
    function fn1() {
    	console.log("fn1:");
    	//fn2();
  	} 
    ...
    <script id="f2" type="text/javascript">
    function fn2() {
    	console.log("fn2:");
    	  fn1();
    	} 
	...
	<script id="dataObjects" type="text/javascript">
	...
	{
		var selectedMeeting = document.getElementById("selectedMeetingID").value;
		console.log("selected meeting ID:" + selectedMeeting);
		return document.getElementById("selectedMeetingID").value;
	}
	function setSelectedMeetingId(meetingId)
	{
	...
	function unhighlight_row(tableName) {
		console.log("unhighlight_row:" + tableName);
		var table = document.getElementById(tableName);
		var rowsNotSelected = table.getElementsByTagName('tr');
		for (var row = 0; row < rowsNotSelected.length; row++) 
		{
			//console.log("unhighlight_row:" + row);
	...
	<script id="openTabs" type="text/javascript">
	openStudents();
	...
	<script id="StudentData" type="text/javascript">
	...
	function addStudent() 
	{
		...
		if ( !studentsLoaded() ) 
		{
			...
			studentDataObject = JSON.parse(JSON.stringify({	"student" : [] }));
		}
		...
		console.log('Now students!!!!' + JSON.stringify(studentDataObject, null, 1));
		cancelAddStudent();
	}
	function addToStudentList() 
	{
		...
		var studentIds = [];
		var selectedMeetingId = getSelectedMeetingId();
		if ( selectedMeetingId !== null && selectedMeetingId.localeCompare("") != 0)
		{
	...
	{
		console.log("update student:" + document.getElementById('selectedStudentID').value);
			var selectedStudent = studentDataObject.students.find( record => record.id === parseInt(document.getElementById('selectedStudentID').value));
			console.log("update student" + selectedStudent.lastName);
			selectedStudent.firstName = document.getElementById('inputFirstName').value;
			selectedStudent.lastName = document.getElementById('inputLastName').value;
			selectedStudent.dob = document.getElementById('inputDOB').value;
			selectedStudent.grade = document.getElementById('inputGrade').value;
			selectedStudent.status = document.getElementById('inputStatus').value;
			loadStudentTable2();
			document.getElementById("studentInputForm").style.display = "none";
			//unhighlight_row('student-table2');
	}

	</script>
	<script id="StudentForm" type="text/javascript">
	function openStudentForm() 
	{
		console.log("open student form" + arguments.length);
		var studentId = document.getElementById('selectedStudentID').value;
		console.log('Update the meeting table:' + studentId );
		if( studentId === null || studentId.localeCompare("") == 0)
		{
			document.getElementById('addStudentFormButton').value  = "Add";
		}
		else
		{
			var selectedStudent = studentDataObject.students.find( record => record.id === parseInt(studentId));
			console.log("open student form" + selectedStudent.lastName);
			document.getElementById('inputFirstName').value = selectedStudent.firstName;
			document.getElementById('inputLastName').value = selectedStudent.lastName;
			document.getElementById('inputDOB').value = selectedStudent.dob;
			document.getElementById('inputGrade').value = selectedStudent.grade;
			document.getElementById('inputStatus').value = selectedStudent.status;
			document.getElementById('selectedStudentID').value = selectedStudent.id;
			document.getElementById('addStudentFormButton').value  = "Update";
		}
		document.getElementById("studentInputForm").style.display = "block";
	}
	function closeStudentForm() 
	{
		console.log("close student form");
		document.getElementById("studentInputForm").style.display = "none";
		//unhighlight_row('student-table2');
	}
	...
	<script id="StudentMeetingTable" type="text/javascript">
	function clearMeetingTable() {
		document.getElementById("student-meeting-table").innerHTML = "";
	}
	function loadStudentMeetingTable() {
		var studentId = document.getElementById('selectedStudentID').value;
		console.log('Update the meeting table:' + studentId );
		if( studentId === null || studentId.localeCompare("") == 0)
		{
			console.log('No load necessary');
		}
		//clearMeetingTable();
		var table = document.getElementById('student-meeting-table');
		var currentLength = table.rows.length;
		for ( x = currentLength; x > 1; x-- ) 
		{
			table.deleteRow( x - 1);
		}
		...
		{//			if( selectedGrade.localeCompare(studentDataObject.students[x].grade) == 0)
			for( var y = 0; y < studentDataObject.meetings[x].studentIDs.length; y++ )
			{
				if( studentDataObject.meetings[x].studentIDs[y] == studentId)
				{
					console.log('MATCH!!!: ' + studentId );
					var newRow = tbodyRef.insertRow();
					newRow.insertCell(0).innerHTML = studentDataObject.meetings[x].date;
					newRow.insertCell(1).innerHTML = studentDataObject.meetings[x].time;
					newRow.insertCell(2).innerHTML = studentDataObject.meetings[x].code;
					newRow.insertCell(3).innerHTML = studentDataObject.meetings[x].notes;
					newRow.insertCell(4).innerHTML = studentDataObject.meetings[x].id;
				}
			}
		}
		highlight_student_meeting_row();
	}
	function highlight_student_meeting_row() {
		var table = document.getElementById('student-meeting-table');
		...
		for (var i = 0; i < cells.length; i++) 
		{
			...
			{
				console.log('highlight_student_meeting_row:');
				...
				if (rowId == 0) {
					...
					selectedMeeting = table.rows.item(rowId).cells.item(4).innerHTML;
					document.getElementById("selectedMeetingID").value = table.rows.item(rowId).cells.item(4).innerHTML;
					console.log('selectedMeeting1:' + selectedMeeting );
					console.log('selectedMeeting1:' + document.getElementById("selectedMeetingID").value );
			...
			cell.ondblclick = function()
			{
				console.log('selectedMeeting2: ' + selectedMeeting);
				openCalendarForm(parseInt(selectedMeeting));
				limitedCalendarForm();
			}
		}
	}
	</script>
<script id="StudentDataTable" type="text/javascript">
//Student Data
	function loadStudentTable2() {
		...
		var studentTable = document.getElementById("student-table2");
		var currentLength = studentTable.rows.length;
		console.log("currentLength:" + currentLength );
		for ( x = currentLength; x > 1; x-- ) 
		{
			//console.log("Delete row:" + (x - 1) );
			studentTable.deleteRow( x - 1);
		}
	...
	function highlight_student_row() {
		var table = document.getElementById('student-table2');
		var cells = table.getElementsByTagName('td');
		for (var i = 0; i < cells.length; i++) 
		{
			// Take each cell
			...
			cell.onclick = function() 
			{
				console.log('highlight_student_row:');
				// Get the row id where the cell exists
				var rowId = this.parentNode.rowIndex;
				if (rowId == 0) {
					console.log('Selected rowId: ' + rowId);
					...
					var selectedStudent = table.rows.item(rowId).cells.item(5).innerHTML;
					...
					closeStudentForm();
					document.getElementById('selectedStudentID').value = selectedStudent;
					loadStudentMeetingTable();
				}
				unhighlight_row('calendar-table2');
			}
		}
	}
</script>
	
	<script id="CalendarData" type="text/javascript">
	...
	function addUpdateMeeting() 
	{
		...
		newMeetingDate = dateStringFormatter(document.getElementById("addUpdateDate").value);
		newMeetingTime = toStandardTime(document.getElementById("meetingTime").value);
		...
		if ( selectedMeetingId !== null && selectedMeetingId.localeCompare("") != 0)
		{
			console.log("UPDATE:" + selectedMeetingId);
			...
			else
			{
				...
				console.log("validMeeting Update:" + JSON.stringify(meetingObject));
				loadCalendarTable2();
				loadStudentMeetingTable();
				closeCalendarForm();
				console.log("!!!!!studentDataObject.meetings:" + studentDataObject.meetings.length);
	...
	<script id="CalendarForm" type="text/javascript">
	function openCalendarForm() 
	{
		console.log("openCalendarForm:" + arguments[0]);
		...
		if ( selectedMeetingId === null || selectedMeetingId.localeCompare("") == 0)
		{
			//document.getElementById("selectedMeetingTime").value = table.rows.item(rowId).cells.item(0).innerHTML;
			//document.getElementById("selectedMeetingDate").value = table.rows.item(1).cells.item(cellId).innerHTML;
			//document.getElementById("meetingTime").value
			//document.getElementById('meetingTime').value = militaryTimeEquiv(arguments[1]);
			console.log("tVal:" + document.getElementById("selectedMeetingTime").value);
			console.log("dVal:" + document.getElementById("selectedMeetingDate").value);
			document.getElementById("addUpdateDate").value = dateStringFormatterYYYYMMDD(document.getElementById("selectedMeetingDate").value);
			//console.log("dtVal:" + dtVal);
			document.getElementById('meetingTime').value = militaryTimeEquiv(document.getElementById("selectedMeetingTime").value);
			//document.getElementById("addUpdateDate").value
			//var dtStr = document.getElementById("addUpdateDate").value;
			//console.log("dtStr:" + dtStr);
			//var dtVal = arguments[0].split("/")[2] + "-" + arguments[0].split("/")[0] + "-" + arguments[0].split("/")[1];
			//dtStr:undefined-2021-02-25-undefined
			var dtVal = document.getElementById("addUpdateDate").value;
			console.log("dtVal:" + dtVal);
			//document.getElementById("addUpdateDate").value = dtVal;
			console.log("openCalendarForm no meeting:" + dtVal);
			document.getElementById('addUpdateMeetingButton').textContent  = "Add Meeting";
		...
		{
			console.log("openCalendarForm for meeting:" + selectedMeetingId);
			//document.getElementById("addUpdateDate").value = militaryTimeEquiv(document.getElementById("selectedMeetingTime").value);//getTodaysDate();
			var selectedMeeting = studentDataObject.meetings.find( record => record.id === parseInt(selectedMeetingId));
			...
			document.getElementById('addUpdateMeetingButton').textContent  = "Update Meeting";
	...
	function closeCalendarForm() 
	{
		...
		document.getElementById('meetingTime').disabled = false;
		document.getElementById('addUpdateDate').disabled = false;
		document.getElementById("codeSelectList").style.visibility = "visible";
		document.getElementById("adminSelectList").style.visibility = "visible";
		document.getElementById("meetingDuration").style.visibility = "visible";
		document.getElementById("studentSelectList").style.visibility = "visible";
		document.getElementById("grade").style.visibility = "visible";

	}
	
	function limitedCalendarForm() 
	{
		document.getElementById('meetingTime').disabled = true;
		document.getElementById('addUpdateDate').disabled = true;
		document.getElementById("codeSelectList").style.visibility = "hidden";
		document.getElementById("adminSelectList").style.visibility = "hidden";
		document.getElementById("meetingDuration").style.visibility = "hidden";
		document.getElementById("studentSelectList").style.visibility = "hidden";
		document.getElementById("grade").style.visibility = "hidden";

		console.log("limitedCalendarForm");
	}


	</script>
<script id="CalendarTable" type="text/javascript">
//Calendar Data
function loadCalendarTable2()
{
	...
	var tbodyRef = document.getElementById("calendar-table2").getElementsByTagName('tbody')[0];
	for( var x = 8; x < 17; x++ )
	{
		var amVal = " AM";
		var pmVal = " PM";
		...
		for( var y = 0; y < hrSegs.length; y++ )
		{
			...
			var newRow = tbodyRef.insertRow();//Add row
			...
			{	
				//Load row
				{
					...
					var meetingInformationStr = id.code;
...
function highlight_calendar_row() {
	...
	for (var i = 0; i < cells.length; i++) {
		...
		cell.ondblclick = function() 
		{
			...
			unhighlight_row('student-table2');
			console.log('selected meeting ID2:' + getSelectedMeetingId());
		}
	}
}
</script>
	<script id="TimeDateFunctions" type="text/javascript">
	function getDate() 
	{
		var today = new Date();
	...
	function dateStringFormatterYYYYMMDD( dateIn )
	{//02/22/2021 to 2021-02-22
		return dateIn.split("/")[2] + "-" + dateIn.split("/")[0] + "-" + dateIn.split("/")[1];
	}
	...
	<script id="ValidateFileReader" >
	//Validate file reader
